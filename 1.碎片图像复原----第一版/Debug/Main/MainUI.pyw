...
if __package__ is None:
    ...
    from Package.Config import (ROOTDIR, SPLITDIR, RECOVERDIR, FILEDIR, FILEPATH, 
                                    FIGNAME, FILENAME, SPLITSNM, IMAGE_SIZE)
else:
    from ..Package.LoadFigure import loadFigure
    from ..Package.SplitFigure import splitFigure
    from ..Package.RecoverFigure import ImgRecover, compareFigure
...
class MainUI(QWidget):
    ...
    def initUI(self):
        ...
        self.pushButton4.setGeometry(700, 25, 200, 45)
        ...
        self.pushButton4.setGeometry(700, 100, 200, 45)
        self.pushButton4.setFont(QFont('SimHei',14))
        ...
        self.label5.move(680, 790)
        self.label5.resize(400, 50)
        self.label5.setFont(QFont('SimHei',18))
        pe = QPalette()
        pe.setColor(QPalette.WindowText, QColor(0, 128, 0))
        self.label5.setPalette(pe)
        # 显示“碎片形状”
        self.label6 = QLabel(self)
        self.label6.move(720, 220)
        self.label6.setFont(QFont('SimHei',18))
        self.label6.setText("切分类型：")
        # 单选框，”均匀直线“
        self.radioButton3 = QRadioButton('均匀直线', self)
        self.radioButton3.move(730, 280)
        self.radioButton3.setFont(QFont('SimHei',12))
        # 单选框，”均匀曲线“
        self.radioButton4 = QRadioButton('均匀曲线', self)
        self.radioButton4.move(730, 320)
        self.radioButton4.setFont(QFont('SimHei',12))
        # 单选框，”随机曲线“
        self.radioButton5 = QRadioButton('随机曲线', self)
        self.radioButton5.move(730, 360)
        self.radioButton5.setFont(QFont('SimHei',12))
        self.radioButton5.setChecked(True)
        # 单选框按钮分组2
        self.group2 =  QButtonGroup(self)
        self.group2.addButton(self.radioButton3, id=21)
        self.group2.addButton(self.radioButton4, id=22)
        self.group2.addButton(self.radioButton5, id=23)
        self.group2.buttonClicked.connect(self.changeSplit)
        # 显示“碎片数目”
        self.label7 = QLabel(self)
        self.label7.move(720, 475)
        self.label7.setFont(QFont('SimHei',18))
        self.label7.setText("切分数目：")
        # 下拉列表，用来选择行数 3, 4, 5
        self.box1 = QComboBox(self)
        self.box1.addItems(("行数:"+str(i) for i in range(1, 6)))
        self.box1.move(677, 550)
        self.box1.setFont(QFont('SimHei',12))
        self.box1.setCurrentIndex(3)
        self.box1.activated[str].connect(self.changeNM)
        # 下拉列表，用来选择列数 3, 4, 5
        self.box2 = QComboBox(self)
        self.box2.addItems(("列数:"+str(i) for i in range(1,6)))
        self.box2.move(817, 550)
        self.box2.setFont(QFont('SimHei',12))
    ...
    def createFigs(self):
        if self.figFile and os.path.exists(self.figFile):
            splitFigure(filepath=self.figFile, isCurve=self.isCurve, isBalance=self.isBalance)
        else:
            loadFigure()
            splitFigure(isCurve=self.isCurve, isBalance=self.isBalance)
    ...
    def recoverFig(self):
        if self.created:
            ...
            accuOrder = imgRecover.getAccuOrder(isCurve=self.isCurve)
            if accuOrder:
                filepath = os.path.join(RECOVERDIR, FILENAME)
                self.showRecoveredFig(filepath)
            else:
    ...
    def computeAccuracy(self):
        ...
        for i, dirname in enumerate(dirList):
            ...
            splitFigure(dirname, orgfile, isCurve=self.isCurve, isBalance=self.isBalance)
            imgRecover = ImgRecover(dirname, RECOVERDIR)
            filename = "sample"+str(i+1)+".jpg"
            accuOrder = imgRecover.getAccuOrder(filename, splited=False, isCurve=self.isCurve)
            if accuOrder:
                newfile = os.path.join(RECOVERDIR, filename)
                succ += compareFigure(orgfile, newfile)
            total += 1
        progress.setValue(len(dirList))
        QMessageBox.information(self, "测试结果", "准确率: {:.2%}".format(succ/total))
        self.label5.setText("准确率: {:.2%}".format(succ/total))
